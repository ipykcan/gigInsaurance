...
export async function getWorkingConditionForecast(lat, lng) {
    ...
    try {
        ...
        if (city || state) {
            try {
                const NEWS_URL = `http://127.0.0.1:5000/news?` + new URLSearchParams({ city, state }).toString();
                const resp = await fetch(NEWS_URL, { method: "GET" });
                if (resp.ok) {
                    const json = await resp.json();
                    if (typeof json?.strikeRiskPercent === 'number') strikeRiskPercentOverride = json.strikeRiskPercent;
                    if (typeof json?.newsRiskPercent === 'number') newsRiskPercentOverride = json.newsRiskPercent;
                    if (Array.isArray(json?.headlines)) newsHighlights = json.headlines;
                }
            } catch (e) {
